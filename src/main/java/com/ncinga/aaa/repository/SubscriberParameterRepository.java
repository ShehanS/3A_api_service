...
public interface SubscriberParameterRepository extends JpaRepository<SubscriberParameterEntity, Integer> {
    @Transactional
    @Modifying
    @Query(value = "DELETE FROM bb_subscriber_parameter WHERE subscriber_id = :id AND nas_id_pattern=:planName", nativeQuery = true)
    int deleteSubscriberPlan(@Param("id") int id, @Param("planName") String planName);
